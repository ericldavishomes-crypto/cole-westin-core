...
EPISODIC_EXTRACTION_MODEL = os.getenv(
    "COLE_EPISODIC_EXTRACTION_MODEL",
    "",
).strip()

EPISODIC_MINIO_BUCKET = os.getenv(
    "COLE_EPISODIC_MINIO_BUCKET",
    "",
).strip()


def get_episodic_minio_bucket() -> str:
    if not EPISODIC_MINIO_BUCKET:
        raise RuntimeError(
            "COLE_EPISODIC_MINIO_BUCKET is required for episodic artifacts"
        )
    return EPISODIC_MINIO_BUCKET


def get_episodic_extraction_model() -> str:
    if not EPISODIC_EXTRACTION_MODEL:
        raise RuntimeError(
            "COLE_EPISODIC_EXTRACTION_MODEL is required for episodic extraction"
        )
    return EPISODIC_EXTRACTION_MODEL
