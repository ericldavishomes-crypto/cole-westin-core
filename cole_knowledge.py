...
if st.session_state.current_tab.strip() == "New Chat":
    ...
    if prompt := st.chat_input("Speak directly to Cole..."):
        ...
        # Safely query your 5 Qdrant vaults for Cole's cognitive scaffolding
        retrieved_memories = cole_knowledge.fetch_cole_memories(prompt)
        dynamic_system_prompt = system_prompt + retrieved_memories

        compiled_messages = [{"role": "system", "content": dynamic_system_prompt}] + [{"role": m["role"], "content": m["content"]} for m in st.session_state.messages if m["role"] != "system"]
